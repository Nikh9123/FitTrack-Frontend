...
const AI_MESSAGES = [
  ...
  "Extracting precise metrics with cloud OCR...",
  "Deep-learning neural networks mapping body composition...",
  "Comparing visceral fat levels to demographic benchmarks...",
  "Synthesizing personalized training recommendations...",
];
...
export default function InBodyScreen() {
  ...
  const { reports, loading: reportsLoading, error: reportsError, refresh, deleteReport } = useReports(token);
  ...
  useEffect(() => {
    if (phase === "analyzing") {
      pulseAnim.value = withRepeat(
        withSequence(withTiming(1.12, { duration: 750 }), withTiming(1, { duration: 750 })),
        -1,
        false
      );
      spinAnim.value = withRepeat(withTiming(1, { duration: 3000 }), -1, false);
      glowAnim1.value = withRepeat(withTiming(1, { duration: 8000 }), -1, true);
      glowAnim2.value = withRepeat(withTiming(1, { duration: 6000 }), -1, true);
      sweepAnim.value = withRepeat(withTiming(1, { duration: 2000 }), -1, true);
      
      const interval = setInterval(() => {
        setMessageIdx((prev) => (prev + 1) % AI_MESSAGES.length);
      }, 2800);
  ...
  useEffect(() => {
    if (phase === "analyzing") {
      smoothProgress.value = withSpring(analyzeProgress, { damping: 20, stiffness: 80 });
    } else {
  ...
  const spinStyle = useAnimatedStyle(() => {
    const rotate = `${spinAnim.value * 360}deg`;
    return {
      transform: [{ rotate }],
    };
  });
  ...
  const glowStyle1 = useAnimatedStyle(() => ({
    transform: [
      { scale: 0.8 + glowAnim1.value * 0.4 },
      { translateX: Math.sin(glowAnim1.value * Math.PI) * 25 },
      { translateY: Math.cos(glowAnim1.value * Math.PI) * 20 },
    ],
    opacity: 0.12 + Math.sin(glowAnim1.value * Math.PI) * 0.05,
  }));

  const glowStyle2 = useAnimatedStyle(() => ({
    transform: [
      { scale: 0.9 + glowAnim2.value * 0.3 },
      { translateX: -Math.cos(glowAnim2.value * Math.PI) * 20 },
      { translateY: -Math.sin(glowAnim2.value * Math.PI) * 25 },
    ],
    opacity: 0.08 + Math.cos(glowAnim2.value * Math.PI) * 0.04,
  }));

  const sweepStyle = useAnimatedStyle(() => ({
    transform: [{ translateY: -55 + sweepAnim.value * 110 }],
    opacity: 0.15 + Math.sin(sweepAnim.value * Math.PI) * 0.55,
  }));
  ...
  const showToast = useCallback((msg: string, type: "success" | "error") => {
    ...
    toastTimer.current = setTimeout(() => {
      ...
      toastTranslateY.value = withTiming(60, { duration: 250 });
  ...
  }, [deleteTarget, deleteReport, showToast]);
      ...
      >
        ...
        {phase === "analyzing" && (
          <View style={styles.analyzingContainer}>
            {/* Animated slow breathing background glows */}
            <View style={StyleSheet.absoluteFillObject} pointerEvents="none">
              <Animated.View style={[styles.bgGlowPurple, { backgroundColor: colors.purple }, glowStyle1]} />
              <Animated.View style={[styles.bgGlowPink, { backgroundColor: colors.primary }, glowStyle2]} />
            </View>

            <ScrollView contentContainerStyle={styles.analyzingScroll} showsVerticalScrollIndicator={false}>
              {/* Premium Glassmorphic Card floating */}
              <GlassCard style={styles.analyzingGlassCard} elevated shadowLevel="strong">
                
                {/* Visual scanner hero container */}
                <View style={styles.scannerWrapper}>
                  {/* Outer Dashed Spinning Ring */}
                  <Animated.View style={[styles.scannerRingOuter, { borderColor: colors.purple + "40" }, spinStyle]} />
                  
                  {/* Middle Solid Thin Ring */}
                  <View style={[styles.scannerRingMiddle, { borderColor: colors.border }]} />
                  
                  {/* Inner Breathing Ring */}
                  <Animated.View style={[styles.scannerRingInner, { borderColor: colors.purple + "60", backgroundColor: colors.purple + "08" }, pulseStyle]}>
                    
                    {/* Scanner horizontal sweep line */}
                    <Animated.View style={[styles.scannerSweepLine, { backgroundColor: colors.purple }, sweepStyle]} />
                    
                    {/* Glowing sparkles in center */}
                    <Ionicons name="sparkles" size={42} color={colors.purple} />
                  </Animated.View>
                </View>

                {/* Typography Hierarchy */}
                <Text style={[colors.typography.h2, { color: colors.foreground, textAlign: "center", fontSize: 22, fontWeight: "800", letterSpacing: -0.5 }]}>
                  Analyzing Body Composition
                </Text>

                {/* Dynamically rotating AI messages */}
                <View style={styles.rotatingMsgContainer}>
                  <Text style={[colors.typography.bodyMedium, { color: colors.mutedForeground, textAlign: "center", lineHeight: 22, minHeight: 44 }]}>
                    {AI_MESSAGES[messageIdx]}
                  </Text>
                </View>

                {/* Progress bar container */}
                <View style={styles.progressContainer}>
                  <View style={[styles.premiumTrack, { backgroundColor: colors.border }]}>
                    <Animated.View style={[styles.premiumFill, fillStyle]}>
                      <LinearGradient
                  ...
                  <View style={styles.progressLabelRow}>
                    <Text style={[colors.typography.caption, { color: colors.mutedForeground }]}>
                      {uploadProgress?.step === "uploading" ? "Uploading report..." :
                       uploadProgress?.step === "ocr" ? "Reading composition data..." :
                       uploadProgress?.step === "processing" ? "Extracting biomarkers..." :
                       "Finalizing health metrics..."}
                    </Text>
                    <Text style={[colors.typography.h3, { color: colors.purple, fontWeight: "800", fontSize: 20 }]}>
                      {analyzeProgress}%
                    </Text>
                  </View>
                </View>
              ...
              <View style={styles.premiumStepsContainer}>
                {[
                  { label: "Uploading InBody Report", done: analyzeProgress > 18, active: uploadProgress?.step === "uploading" },
                  { label: "Extracting metrics with OCR", done: analyzeProgress > 42, active: uploadProgress?.step === "ocr" },
                  { label: "Analyzing with Gemini AI", done: analyzeProgress > 68, active: uploadProgress?.step === "processing" },
                  { label: "Generating personalized plan", done: analyzeProgress >= 100, active: uploadProgress?.step === "done" },
                ].map((step, idx) => {
                  const isCurrent = step.active || (!step.done && (idx === 0 || (idx > 0 && [
                    analyzeProgress > 18,
                    analyzeProgress > 42,
                    analyzeProgress > 68,
                    analyzeProgress >= 100
                    ...
                    >
                      <View style={[
                        ...
                        <Ionicons
                          ...
                          size={15}
                          color={step.done ? colors.green : isCurrent ? colors.purple : colors.mutedForeground}
                        />
                      </View>
                      <Text style={[
                        ...
                        { 
                          flex: 1, 
                          color: step.done ? colors.foreground : isCurrent ? colors.foreground : colors.mutedForeground,
      ...
      <Modal visible={deleteTarget !== null} transparent animationType="fade">
        <View style={styles.popupOverlay}>
          ...
          >
            ...
            {deleteTarget && (
              <Text style={[colors.typography.caption, { color: colors.mutedForeground, textAlign: "center" }]}>
                {new Date(deleteTarget.createdAt).toLocaleDateString("en-IN", {
                  day: "2-digit",
                  month: "long",
                  year: "numeric",
                })}
            ...
            <View style={styles.deleteModalBtns}>
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text style={[colors.typography.bodyMedium, { color: colors.foreground }]}>Cancel</Text>
              </TouchableOpacity>
              ...
              >
                {isDeleting ? (
                  ...
                  <>
                    ...
                    <Text style={[colors.typography.bodyMedium, { color: "#fff" }]}>Delete</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
      ...
      {toast && (
        <Animated.View
          style={[
            styles.toast,
            {
              backgroundColor: toast.type === "success" ? colors.green + "F2" : colors.destructive + "F2",
        ...
        >
          <Ionicons
            ...
            color="#fff"
          />
          <Text style={[colors.typography.bodyMedium, { color: "#fff", flex: 1 }]}>
            {toast.msg}
          </Text>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  // Redesigned loading/analyzing screen styles
  analyzingContainer: {
    flex: 1,
    width: "100%",
  },
